...
Node *find_user(QNode *current, char *name){
    ...
    // Store the current opposite.
    QNode* temp_other_way = other_way;
    ...
    if (current->node_type == REGULAR){
        ...
        // Update other_way.
        other_way = other_way->children[1].qchild;
        ...
        // Reset other_way.
        other_way = temp_other_way;
        // Update other_way.
        other_way = other_way->children[0].qchild;
    ...
    // current is a leaf node
        ...
        // Choose list from other_way.
        opposite = other_way->children[1].fchild;
        ...
        // Choose list from other_way.
        opposite = other_way->children[0].fchild;
...
// Precondition: we have just called find_user.
...
Node *get_opposite_list(QNode *current, char* answer) {
    ...
    // current is an inner node
    if (current->node_type == REGULAR){
        return get_opposite_list(current->children[walk].qchild, answer+1);
    // current is a leaf node
    } else {
        return current->children[walk].fchild;
    }
}